...
if [ "$WSL_MODE" = true ]; then
    ...
    if check_wsl_existing_config; then
        echo -e "${GREEN}✓ dnsmasq wildcard configured${NC}"
        ...
        local resolv_ok=false
        if grep -q "^nameserver 127.0.0.1" /etc/resolv.conf 2>/dev/null; then
            resolv_ok=true
            echo -e "${GREEN}✓ resolv.conf configured${NC}"
        else
            echo -e "${YELLOW}⚠ resolv.conf not pointing to local dnsmasq${NC}"
        fi
        ...
        if ping -c 1 -W 2 test.test > /dev/null 2>&1; then
            ...
            echo -e "  ${YELLOW}○${NC} *.test not resolving"
        fi

        # If DNS not working and resolv.conf is wrong, fix it
        if [ "$resolv_ok" = false ]; then
            echo ""
            echo -e "${YELLOW}Fixing resolv.conf...${NC}"

            # Ensure wsl.conf has generateResolvConf = false
            if [ -f "$WSL_CONF" ]; then
                if grep -q "^\[network\]" "$WSL_CONF" 2>/dev/null; then
                    if ! grep -q "generateResolvConf" "$WSL_CONF" 2>/dev/null; then
                        ...
                        echo -e "${GREEN}✓${NC} Added generateResolvConf to wsl.conf"
                    fi
                else
                    echo "" >> "$WSL_CONF"
                    echo "[network]" >> "$WSL_CONF"
                    echo "generateResolvConf = false" >> "$WSL_CONF"
                    echo -e "${GREEN}✓${NC} Added [network] section to wsl.conf"
                fi
            else
                cat > "$WSL_CONF" << WSLEOF
[boot]
...
WSLEOF
            ...
            # Remove old resolv.conf (might be a symlink) and create new one
            rm -f /etc/resolv.conf
            cat > /etc/resolv.conf << RESOLVEOF
# Generated by docker-local for WSL2
# Primary: local dnsmasq for *.test domains
nameserver 127.0.0.1
# Fallback: Docker Desktop DNS
nameserver $DOCKER_DESKTOP_DNS
RESOLVEOF
            echo -e "${GREEN}✓${NC} Fixed /etc/resolv.conf"
            ...
            if ping -c 1 -W 2 test.test > /dev/null 2>&1; then
                ...
                echo -e "  ${YELLOW}○${NC} *.test still not resolving"
                echo -e "  ${DIM}You may need to restart WSL: wsl --shutdown${NC}"
            fi
            ...
            # resolv.conf is OK but DNS still not working
            echo ""
            ...
            sleep 1
            if ping -c 1 -W 2 test.test > /dev/null 2>&1; then
                echo -e "${GREEN}✓${NC} *.test now resolving"
            else
                echo -e "${YELLOW}○${NC} *.test still not resolving"
                echo -e "${DIM}You may need to restart WSL: wsl --shutdown${NC}"
            fi
